...
export default function Home({ params }: { params: { name: string } }) {
  ...
  function handleInputChange(
    ...
    const newInputs = inputs.map((input) =>
      input.id === num
        ? { ...input, [event.target.name]: parseFloat(event.target.value) || 0 }
        : input,
    );
    setInputs(newInputs);
  }
  ...
  return (
    <form onSubmit={evaluate} className="flex flex-col m-4 p-4">
      ...
      <Link
        className="me-3 hover:underline hover:text-pes"
          ...
          <div key={key} className="flex flex-col">
            ...
            <div className="flex flex-wrap">
              <label htmlFor="" className="flex flex-col w-72">
                ...
                <input
                  id={`${index.id + 1}`}
                  onChange={(event) => handleInputChange(index.id, event)}
                  name="task"
                  required
                  className="border m-1 px-16 py-2 rounded"
                  type="text"
                />
              </label>
      ...
      <button className="bg-pes w-fit my-3 me-2 rounded text-white px-16 py-3">
        Done
      </button>
      <div className="flex my-1">
        <LoadingButton
          className="bg-pes w-fit me-4 rounded text-white px-16 py-3"
        ...
        <Link
          href="/downloadables/factored-estimate.docx"
          className="bg-pes w-fit me-4 rounded text-white px-16 py-3"
        >
          Print form for survey
        </Link>
      </div>
    </form>
  );
