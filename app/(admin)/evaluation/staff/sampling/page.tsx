...
const WorkSamplingPageInner: React.FC = () => {
  ...
  const [studyMeta, setStudyMeta] = useState<StudyMeta>({ org: '', department: '', analyst: '' });
  const [positions, setPositions] = useState<Position[]>([]);
  const [observations, setObservations] = useState<Observation[]>([]);
  const [studyParameters, setStudyParameters] = useState<StudyParameters>(DEFAULT_PARAMS);
  const [newPosition, setNewPosition] = useState({ name: '', department: '', performanceAllowance: 15 });
  const [newObservation, setNewObservation] = useState({
    positionId: '', date: new Date().toISOString().split('T')[0],
    time: new Date().toTimeString().slice(0, 5), isBusy: true, performanceRating: 100, notes: '',
  });
  ...
  const [currentUser, setCurrentUser] = useState<{ name?: string; org?: string; dept?: string; role?: string } | null>(null);
  ...
  useEffect(() => {
    const user = getCurrentUser<{ name?: string; org?: string; dept?: string; role?: string }>();
    if (!user) return;
    setCurrentUser(user);
  ...
  const confirmDeleteStudy = async () => {
    ...
    try {
      const res = await fetch(`/api/workSampling/studies/${deleteConfirmId}`, { method: 'DELETE' });
  ...
  useEffect(() => {
    ...
    fetch(`/api/workSampling/studies/${idParam}`)
      ...
      .then(json => {
        ...
        setStudyMeta({
          org: study.org ?? '', department: study.department ?? '',
          analyst: study.analyst ?? '',
        });
        // Restore locked schedule from DB (handle legacy integers or new date strings)
        const dbLockedDates: any[] = Array.isArray(study.lockedDates) ? study.lockedDates : [];
        const dbLockedTimes: string[] = Array.isArray(study.lockedTimes) ? study.lockedTimes : [];
        ...
        setStudyParameters(prev => ({
          ...
          studyMonths: Array.isArray(study.studyMonths) && study.studyMonths.length > 0 ? study.studyMonths : (study.studyMonth ? [study.studyMonth] : prev.studyMonths),
          observationsPerDay: study.observationsPerDay ?? prev.observationsPerDay,
          workingHoursPerDay: study.workingHoursPerDay ?? prev.workingHoursPerDay,
          workStartTime: study.workStartTime ?? prev.workStartTime,
          minCycleDuration: study.minCycleDuration ?? prev.minCycleDuration,
          availableAnnualHours: study.availableAnnualHours ?? prev.availableAnnualHours,
          defaultPerformanceAllowance: study.defaultPerformanceAllowance ?? prev.defaultPerformanceAllowance,
        }));
        const mappedPositions = dbPos.map((p: any) => ({
          id: String(p.id), dbId: p.id,
        ...
        setObservations(dbObs.map((o: any) => ({
          id: String(o.id), dbId: o.id,
          positionId: String(o.positionId),
          date: o.date, time: o.time,
          isBusy: o.isBusy, performanceRating: o.performanceRating ?? 100,
          notes: o.notes ?? '',
        })));
      })
      .catch(console.error)
      .finally(() => {
        ...
        setTimeout(() => { isLoadingFromDb.current = false; }, 100);
      });
  ...
  useEffect(() => {
    const mergedMeta = {
      ...
      department: selectedPosition ? selectedPosition.department : studyMeta.department,
    };
    latestSavePayload.current = { studyDbId, studyParameters, studyMeta: mergedMeta };
  ...
  useEffect(() => {
    ...
    const a = accuracy / 100, p = studyParameters.preliminaryP, k = zValue;
    ...
    setStudyParameters(prev => ({
      ...prev, desiredAccuracy: accuracy,
      totalObservations: N, maxDuration: Math.round(B * 100) / 100, estimatedStudyDays: studyDays,
    }));
  }, [
    studyParameters.confidenceLevel, studyParameters.preliminaryP,
    studyParameters.observationsPerDay, studyParameters.workingHoursPerDay,
    studyParameters.minCycleDuration,
  ]);

  // Sync default PA to new-position form
  useEffect(() => {
    setNewPosition(prev => ({ ...prev, performanceAllowance: studyParameters.defaultPerformanceAllowance }));
  ...
  useEffect(() => {
    if (selectedPositionId) {
      setNewObservation(prev => ({ ...prev, positionId: selectedPositionId, date: '', time: '' }));
    }
  }, [selectedPositionId]);
  ...
  const ensureStudy = useCallback(async (): Promise<number> => {
    ...
    const { studyDbId: currentId, studyParameters: sp, studyMeta: sm } = latestSavePayload.current;
    ...
    latestSavePayload.current = { ...latestSavePayload.current, studyDbId: newId };
  ...
  const addPosition = async () => {
    ...
    const optimistic: Position = {
      id: localId, name: newPosition.name.trim(),
    ...
    setNewPosition({ name: '', department: '', performanceAllowance: studyParameters.defaultPerformanceAllowance });
    setError(null);
    ...
    try {
      ...
      const res = await fetch('/api/workSampling/positions', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          studyId: sid, name: optimistic.name,
  ...
  const addObservation = async () => {
    ...
    if (existing) {
      ...
      setObservations(prev => prev.map(o =>
        o.id === localId
          ? { ...o, isBusy: newObservation.isBusy, performanceRating: newObservation.performanceRating, notes: newObservation.notes }
          : o
      ));
      setError(null);
      ...
      try {
        ...
        setSaveStatus('error');
        // Rollback
        setObservations(prev => prev.map(o => o.id === localId ? prevObservation : o));
    ...
    try {
      ...
      const res = await fetch('/api/workSampling/observations', {
        ...
        body: JSON.stringify({
          ...
          date: optimistic.date, time: optimistic.time,
  ...
  const scheduleSave = useCallback(() => {
    ...
    paramsSaveTimer.current = setTimeout(async () => {
      const { studyDbId: id, studyParameters: sp, studyMeta: sm } = latestSavePayload.current;
  ...
  const generateRandomSchedule = useMemo(() => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      cur += A + (B - A) * R;
      if (cur < end) {
        times.push(`${Math.floor(cur / 60).toString().padStart(2,'0')}:${Math.round(cur % 60).toString().padStart(2,'0')}`);
      }
    }
    return times;
  }, [studyParameters.observationsPerDay, studyParameters.minCycleDuration, studyParameters.maxDuration,
      studyParameters.workStartTime, studyParameters.workingHoursPerDay]);
  ...
  const generateRandomDates = useMemo(() => {
    ...
    const totalK = studyMonths.reduce((sum, m) => sum + (MONTH_DAY_CONSTANTS[m] || 30), 0);
    ...
    studyMonths.forEach(studyMonth => {
      ...
      while (monthDates.length < Math.min(targetDays, 15) && attempts < targetDays * 10) {
        attempts++;
        ...
        if (d.getMonth() === studyMonth - 1 && dow !== 0 && dow !== 6
            && !monthDates.includes(dateStr) && !excludedDates.includes(dateStr)) {
    ...
    return dates.sort();
  }, [studyParameters.studyMonths, studyParameters.estimatedStudyDays, excludedDates]);

  // Keep refs in sync so saveAndLockParameters can read them without stale closure
  useEffect(() => { latestScheduleDates.current = generateRandomDates; }, [generateRandomDates]);
  useEffect(() => { latestScheduleTimes.current = generateRandomSchedule; }, [generateRandomSchedule]);
  ...
  const analysisResults: AnalysisResult[] = positions.map(pos => {
    ...
    const avgPerf = total > 0 ? posObs.reduce((s, o) => s + o.performanceRating, 0) / total : 0;
    ...
    return {
      ...
      utilizationFactor: Ui * 100, busyCount, totalObservations: total,
      avgPerformanceRating: avgPerf, performanceAllowance: pos.performanceAllowance,
      estimatedAnnualManHours: EAMi, estimatedBasicManHours: EBMi, estimatedStandardManHours: ESMi,
    };
  });
  ...
  const SaveIndicator = () => (
    <span className={`text-xs font-medium px-3 py-1 rounded-full ${
      ...
      saveStatus === 'error'  ? 'bg-red-100 text-red-700' : 'hidden'
    }`}>
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="animate-spin rounded-full h-10 w-10 border-b-2" style={{ borderColor: '#322b80' }} />
    </div>
  );
    ...
    <div className="min-h-screen p-6" style={{ background: 'linear-gradient(135deg,#f8fafc 0%,#e2e8f0 100%)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        {isListView ? (
          <div>
            <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
              <div>
                <h1 className="text-3xl font-bold" style={{ color: '#322b80' }}>Work Sampling Studies</h1>
                <p className="text-gray-500 text-sm mt-1">Manage all work sampling analyses</p>
              </div>
              <button onClick={() => { setIsListView(false); setStudyDbId(null); setPositions([]); setObservations([]); setStudyParameters(DEFAULT_PARAMS); }}
            ...
            <div className="bg-white rounded-xl shadow p-6">
              {studyList.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">No work sampling studies found.</p>
                  <button onClick={() => { setIsListView(false); setStudyDbId(null); setPositions([]); setObservations([]); setStudyParameters(DEFAULT_PARAMS); }} className="mt-4 text-indigo-600 underline">Create one now</button>
                </div>
              ) : (
                    ...
                    <tbody className="divide-y">
                      {studyList.map((study) => (
                        <tr key={study.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-4 py-3 font-semibold text-gray-800">#{study.id}</td>
                          <td className="px-4 py-3">{study.org || '—'}</td>
                          <td className="px-4 py-3">{study.department || '—'}</td>
                          <td className="px-4 py-3">{study.analyst || '—'}</td>
                          <td className="px-4 py-3">
                            {Array.isArray(study.studyMonths) && study.studyMonths.length > 0 
                              ? study.studyMonths.map((m: number) => MONTH_NAMES[m-1]).join(', ')
                              : (study.studyMonth ? MONTH_NAMES[study.studyMonth-1] : '—')}
                          </td>
                          <td className="px-4 py-3 font-medium text-indigo-600">{study.positionCount || 0}</td>
                          <td className="px-4 py-3 flex items-center gap-4">
                            <a href={`/evaluation/staff/sampling?studyId=${study.id}`}
                               className="text-indigo-600 hover:text-indigo-800 font-medium flex items-center gap-1">
                              Open <ChevronRight size={14} />
                            </a>
                            <button onClick={() => setDeleteConfirmId(study.id)} className="text-red-500 hover:text-red-700 transition-colors" title="Delete Study">
                              <Trash2 size={16} />
                            </button>
                          </td>
                        </tr>
          ...
          <>
            ...
            <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
              <div>
                <h1 className="text-3xl font-bold" style={{ color: '#322b80' }}>Work Sampling Analysis</h1>
                <p className="text-gray-500 text-sm mt-1">
                  {studyDbId ? `Study #${studyDbId}` : 'New study — will be saved when you add a position'}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <SaveIndicator />
                {studyDbId && (
                  <button onClick={() => { setIsListView(true); setStudyDbId(null); router.push('/evaluation/staff/sampling'); }} className="text-xs text-indigo-600 underline border-none bg-transparent cursor-pointer">
                    &larr; Back to List
                  </button>
                )}
                {studyDbId && (
                  <button onClick={() => { setIsListView(false); setStudyDbId(null); setPositions([]); setObservations([]); setStudyParameters(DEFAULT_PARAMS); router.push('/evaluation/staff/sampling'); }} className="text-xs text-indigo-600 underline border-none bg-transparent cursor-pointer ml-3">
                    + New study
                  </button>
                )}
              </div>
          ...
          {([
            ...
            <button key={key} onClick={() => setActiveTab(key)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium text-sm transition-all ${
                activeTab === key ? 'text-white shadow' : 'text-gray-600 hover:bg-gray-50'
              }`}
              style={{ backgroundColor: activeTab === key ? '#322b80' : 'transparent' }}
            >
              <Icon size={16} />{label}
              {done ? (
                <CheckCircle2 size={14} className={activeTab === key ? 'text-green-300' : 'text-green-500'} />
              ) : (
        ...
        <div className="bg-white rounded-xl shadow-xl p-8">
          ...
          {activeTab === 'positions' && (
            <div>
              <h2 className="text-2xl font-bold mb-2 flex items-center gap-2" style={{ color: '#322b80' }}>
                <Users size={24} /> Position Management
              </h2>
              <p className="text-sm text-gray-500 mb-6">
                Click a position to select it — the Parameters, Observations and Analysis tabs will focus on it.
              </p>

              <div className="grid md:grid-cols-4 gap-4 mb-8 p-6 bg-gray-50 rounded-lg">
                <input type="text" placeholder="Position Name" value={newPosition.name}
                ...
                <input type="text" placeholder="Department" value={newPosition.department}
                  onChange={e => setNewPosition(p => ({ ...p, department: e.target.value }))}
                  className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent" />
                <input type="number" placeholder="PA %" min={0} max={100}
                  value={newPosition.performanceAllowance}
                  onChange={e => setNewPosition(p => ({ ...p, performanceAllowance: Number(e.target.value) }))}
                  className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent"
                ...
                {error && <div className="md:col-span-4 text-red-500 text-sm font-medium">{error}</div>}
              </div>

              <div className="grid gap-3">
                {positions.length === 0
                  ? <p className="text-gray-400 text-center py-10">No positions yet. Add your first position above.</p>
                  : positions.map(pos => {
                    ...
                    return (
                      <div key={pos.id}
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          {/* Selection indicator */}
                          <div className={`w-3 h-3 rounded-full flex-shrink-0 ${isSelected ? 'bg-indigo-600' : 'bg-gray-300'}`} />
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            <span className="px-2 py-1 rounded-full font-medium"
                              style={{ backgroundColor: '#322b8015', color: '#322b80' }}>
                              PA: {pos.performanceAllowance}%
                            </span>
                            <span className="text-gray-400">
                              {posObs.length} obs · {busyCount} busy
                            </span>
                            {!pos.dbId && <span className="text-xs text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded">saving…</span>}
                          </div>
                          {isSelected && (
                        ...
                        <button onClick={e => { e.stopPropagation(); removePosition(pos.id); }}
                ...
                <div className="text-sm text-gray-500">
                  {positions.length === 0 ? (
                    <span className="flex items-center gap-1.5 text-amber-600">
                      <AlertCircle size={14} /> Add at least one position to continue
                    </span>
                  ) : !selectedPosition ? (
                    <span className="flex items-center gap-1.5 text-amber-600">
                      <AlertCircle size={14} /> Click a position above to select it
                    </span>
                  ) : (
                    <span className="flex items-center gap-1.5 text-green-600">
                      <CheckCircle2 size={14} /> <strong>{selectedPosition.name}</strong> selected — {positions.length} position{positions.length !== 1 ? 's' : ''} total
                    </span>
                  )}
                </div>
                <button
          ...
          {activeTab === 'parameters' && (
            <div>
              <div className="mb-6">
                <h2 className="text-2xl font-bold flex items-center gap-2" style={{ color: '#322b80' }}>
                  <Calculator size={24} /> Study Parameters
                  {!studyDbId && <span className="text-sm font-normal text-gray-400 ml-2">(saved when you add a position)</span>}
                </h2>
                {selectedPosition ? (
                  <p className="text-sm text-gray-500 mt-1">
                    These parameters apply to the whole study. Currently viewing in context of{' '}
                    <span className="font-semibold text-indigo-700">{selectedPosition.name}</span>
                    {' '}(PA: {selectedPosition.performanceAllowance}%).
                  </p>
                ) : (
                  <p className="text-sm text-amber-600 mt-1">
                    No position selected — go to the Positions tab and click a position first.
                  </p>
                )}
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <div className="px-5 py-3 font-semibold text-white text-sm" style={{ backgroundColor: '#1e1b4b' }}>
                      Study Info
                    </div>
                    <div className="p-5 space-y-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">Auto-filled from your account</p>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700">
                          Department
                          {selectedPosition && <span className="ml-2 text-xs text-indigo-600">(from selected position)</span>}
                        </label>
                        <input type="text"
                          readOnly={!!selectedPosition}
                          value={selectedPosition ? selectedPosition.department : studyMeta.department}
                          onChange={e => setStudyMeta(prev => ({ ...prev, department: e.target.value }))}
                          placeholder="Select a position to auto-fill"
                          className={`mt-1 w-full px-4 py-2.5 border rounded-lg ${
                            selectedPosition ? 'border-gray-200 bg-gray-100 text-gray-500 cursor-not-allowed' : 'border-gray-300'
                          }`} />
                      </div>
                      <div>
                  ...
                  {/* A: Sampling Accuracy */}
                  <div className="border rounded-lg overflow-hidden">
                    <div className="px-5 py-3 font-semibold text-white text-sm" style={{ backgroundColor: '#322b80' }}>A. Sampling Accuracy</div>
                    <div className="p-5 space-y-4">
                      <label className="block text-sm font-medium text-gray-700">Confidence Level (%)
                        <select value={studyParameters.confidenceLevel}
                          onChange={e => setStudyParameters(p => ({ ...p, confidenceLevel: Number(e.target.value) }))}
                      ...
                      <label className="block text-sm font-medium text-gray-700">Desired Accuracy (%) — auto
                        <div className="relative mt-1">
                          <input readOnly value={studyParameters.desiredAccuracy}
                            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg bg-gray-100 text-gray-500 cursor-not-allowed" />
                          <Info size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" />
                        </div>
                      </label>
                      <label className="block text-sm font-medium text-gray-700">Preliminary P (Proportion Busy)
                        <input type="number" step="0.01" min="0.01" max="0.99"
                          value={studyParameters.preliminaryP}
                          onChange={e => setStudyParameters(p => ({ ...p, preliminaryP: Number(e.target.value) }))}
                          className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                      </label>
                    </div>
                  </div>
                  ...
                  {/* B: Schedule */}
                  <div className="border rounded-lg overflow-hidden">
                    <div className="px-5 py-3 font-semibold text-white text-sm" style={{ backgroundColor: '#4338a8' }}>B. Schedule Configuration</div>
                    <div className="p-5 space-y-4">
                      <div className="block text-sm font-medium text-gray-700">Study Months
                        <div className="mt-2 grid grid-cols-3 gap-2">
                          {MONTH_NAMES.map((n, i) => {
                            const monthIndex = i + 1;
                            const isSelected = studyParameters.studyMonths.includes(monthIndex);
                            return (
                              <label key={i} className="flex items-center gap-2 text-sm font-normal cursor-pointer bg-gray-50 px-3 py-2 rounded border hover:bg-gray-100">
                                <input type="checkbox"
                                  checked={isSelected}
                                  onChange={(e) => {
                                    setStudyParameters(p => {
                                      const next = e.target.checked
                                        ? [...p.studyMonths, monthIndex].sort((a,b)=>a-b)
                                        : p.studyMonths.filter(m => m !== monthIndex);
                                ...
                                {n} <span className="text-gray-400 text-xs">(k={MONTH_DAY_CONSTANTS[monthIndex]})</span>
                              </label>
                            );
                          })}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <label className="block text-sm font-medium text-gray-700">Observations/Day (n)
                          <input type="number" min={1} max={50} value={studyParameters.observationsPerDay}
                            onChange={e => setStudyParameters(p => ({ ...p, observationsPerDay: Number(e.target.value) }))}
                            className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                        </label>
                        <label className="block text-sm font-medium text-gray-700">Working Hours/Day (W)
                          <input type="number" min={1} max={24} value={studyParameters.workingHoursPerDay}
                            onChange={e => setStudyParameters(p => ({ ...p, workingHoursPerDay: Number(e.target.value) }))}
                            className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                        </label>
                        <label className="block text-sm font-medium text-gray-700">Work Start Time (Y₀)
                          <input type="time" value={studyParameters.workStartTime}
                            onChange={e => setStudyParameters(p => ({ ...p, workStartTime: e.target.value }))}
                            className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                        </label>
                        <label className="block text-sm font-medium text-gray-700">Min Cycle Duration A (min)
                          <input type="number" min={1} max={120} value={studyParameters.minCycleDuration}
                            onChange={e => setStudyParameters(p => ({ ...p, minCycleDuration: Number(e.target.value) }))}
                            className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                        </label>
                      </div>
                      <div className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 border text-sm">
                        <span className="text-gray-600">Max Duration (B):</span>
                        <span className="font-semibold" style={{ color: '#322b80' }}>{studyParameters.maxDuration.toFixed(1)} min</span>
                        <span className="text-xs text-gray-400 ml-auto">B = (2W×60 − n×A) / n</span>
                      </div>
                      {studyParameters.maxDuration <= studyParameters.minCycleDuration && (
                        <p className="text-amber-600 text-sm bg-amber-50 p-2 rounded">B must be greater than A — reduce observations/day or increase working hours.</p>
                      )}
                    </div>
                  </div>
                  ...
                  {/* C: Work Year */}
                  <div className="border rounded-lg overflow-hidden">
                    <div className="px-5 py-3 font-semibold text-white text-sm" style={{ backgroundColor: '#5b4fc7' }}>C. Work Year &amp; Allowance</div>
                    <div className="p-5 space-y-4">
                      <label className="block text-sm font-medium text-gray-700">Available Annual Hours
                        <input type="number" min={0} value={studyParameters.availableAnnualHours}
                          onChange={e => setStudyParameters(p => ({ ...p, availableAnnualHours: Number(e.target.value) }))}
                          className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                        <span className="text-xs text-gray-400">Standard: 2080 hrs</span>
                      </label>
                      <label className="block text-sm font-medium text-gray-700">Default Performance Allowance (PA%)
                        <input type="number" min={0} max={100} value={studyParameters.defaultPerformanceAllowance}
                          onChange={e => setStudyParameters(p => ({ ...p, defaultPerformanceAllowance: Number(e.target.value) }))}
                          className="mt-1 w-full px-4 py-2.5 border border-gray-300 rounded-lg" />
                      </label>
                    </div>
                  </div>
                  ...
                  {/* Save & Lock button */}
                  <div className={`border rounded-lg p-5 ${paramsSaved ? 'border-green-300 bg-green-50' : 'border-indigo-200 bg-indigo-50'}`}>
                    {paramsSaved ? (
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 text-green-700 font-semibold">
                          Parameters saved — study is ready for observation entry
                        </div>
                        <p className="text-xs text-green-600">
                          {lockedDates.length} study dates and {lockedTimes.length} daily time slots are locked in.
                          Go to the <button onClick={() => setActiveTab('observations')} className="underline font-semibold">Observations tab</button> to start recording.
                        </p>
                        <button onClick={() => { setParamsSaved(false); setLockedDates([]); setLockedTimes([]); }}
                      ...
                      <div className="space-y-3">
                        <p className="text-sm text-indigo-800">
                          When you're happy with the parameters above, save them to lock the study schedule.
                          This will generate and store the observation dates and daily time slots.
                        </p>
                        {positions.length === 0 && (
                          <p className="text-xs text-amber-600">Add at least one position before saving.</p>
                        )}
                        <button
                          ...
                          style={{ backgroundColor: '#322b80' }}>
                          {saveStatus === 'saving' ? 'Saving…' : 'Save & Lock Study Parameters'}
                        </button>
                      </div>
                    )}
                  </div>
                ...
                <div className="space-y-6">
                  <div className="bg-gray-50 p-6 rounded-lg sticky top-6 space-y-3">
                    <h3 className="font-semibold text-base" style={{ color: '#322b80' }}>Calculated Requirements</h3>
                    {[
                      ['K (std deviations)', CONFIDENCE_ACCURACY_MAP[studyParameters.confidenceLevel]?.zValue ?? '—'],
                      ['Required Observations (N)', studyParameters.totalObservations.toLocaleString()],
                      ['Estimated Study Duration', `${studyParameters.estimatedStudyDays} days`],
                      ['Max Duration (B)', `${studyParameters.maxDuration.toFixed(1)} min`],
                    ].map(([label, val]) => (
                      <div key={label as string} className="flex justify-between items-center p-3 bg-white rounded-lg border border-gray-100 text-sm">
                        <span className="text-gray-600">{label}</span>
                        <span className="font-bold" style={{ color: '#322b80' }}>{val}</span>
                      </div>
                    ))}
                    {studyParameters.confidenceLevel === 95 && (
                      <div className="px-3 py-2 rounded-lg text-xs text-indigo-700 bg-indigo-50 border border-indigo-100">
                        ...
                        <br />= 1600 × {(1 - studyParameters.preliminaryP).toFixed(3)} / {studyParameters.preliminaryP.toFixed(3)}
                        {' '}= <strong>{Math.ceil(1600 * (1 - studyParameters.preliminaryP) / studyParameters.preliminaryP)}</strong>
                      </div>
                    )}

                    {/* Sample times */}
                    <div className="border-t pt-4 mt-2">
                      <p className="text-sm font-medium mb-2" style={{ color: '#322b80' }}>
                        Sample Times ({studyParameters.observationsPerDay} obs/day)
                      </p>
                      <div className="grid grid-cols-2 gap-1.5">
                        {generateRandomSchedule.map((t, i) => (
                          <div key={i} className="flex items-center gap-1.5 p-1.5 bg-gray-50 rounded text-xs">
                            <span className="w-4 h-4 rounded-full flex items-center justify-center text-white text-xs font-medium"
                              style={{ backgroundColor: '#322b80', fontSize: '9px' }}>{i + 1}</span>
                    ...
                    <div className="border-t pt-4">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-sm font-medium" style={{ color: '#322b80' }}>
                          Study Dates
                        </p>
                        <span className="text-xs text-gray-400">{generateRandomDates.length} days total</span>
                      </div>
                      
                      {studyParameters.studyMonths.sort((a,b)=>a-b).map(month => {
                        // Filter the generated dates for this month
                        const monthStr = String(month).padStart(2, '0');
                        const monthDates = generateRandomDates.filter(d => d.split('-')[1] === monthStr);
                        if (monthDates.length === 0) return null;
                        
                        return (
                          <div key={month} className="mb-4 last:mb-0">
                            <p className="text-xs font-semibold text-gray-500 mb-1">{MONTH_NAMES[month - 1]}</p>
                            <div className="flex flex-wrap gap-1.5">
                              {monthDates.map(dateStr => {
                                const [y, m, d] = dateStr.split('-');
                                const dateObj = new Date(Number(y), Number(m) - 1, Number(d));
                                const dn = dateObj.toLocaleDateString('en-US', { weekday: 'short' });
                                return (
                                  <div key={dateStr} onClick={() => setExcludedDates(p => [...p, dateStr])}
                                    ...
                                    <span className="group-hover:text-red-700">{dn} {Number(d)}</span>
                      ...
                      <p className="text-xs text-gray-400 mt-2">Click to exclude (e.g. public holidays)</p>
                      {excludedDates.length > 0 && (
                        <div className="mt-2 border-t pt-2">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-xs text-red-600 font-medium">Excluded dates</span>
                            <button onClick={() => setExcludedDates([])} className="text-xs text-gray-500 flex items-center gap-1">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {excludedDates.sort().map(dateStr => {
                                const [y, m, d] = dateStr.split('-');
                                const dateObj = new Date(Number(y), Number(m) - 1, Number(d));
                                const dn = dateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short' });
                                return (
                                  <div key={dateStr} onClick={() => setExcludedDates(p => p.filter(d => d !== dateStr))}
            ...
            <div>
              <div className="mb-6">
                ...
                {selectedPosition ? (
                  <p className="text-sm text-gray-500 mt-1">
                    ...
                    <span className="font-semibold text-indigo-700">{selectedPosition.name}</span>.
                    {' '}Switch position from the <button onClick={() => setActiveTab('positions')}
                      className="underline text-indigo-600 hover:text-indigo-800">Positions tab</button>.
                  </p>
                ) : (
                  <p className="text-sm text-amber-600 mt-1">
                    No position selected — go to the{' '}
                    <button onClick={() => setActiveTab('positions')} className="underline text-amber-700">Positions tab</button>
              ...
              {positions.length === 0 ? (
                ...
                <>
                  ...
                  {paramsSaved && (lockedDates.length > 0 || lockedTimes.length > 0) ? (
                    <div className="mb-6 p-4 rounded-lg border border-indigo-200 bg-indigo-50">
                      <p className="text-sm font-semibold text-indigo-800 mb-3">Study Schedule (from saved parameters)</p>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {lockedDates.map(dateStr => {
                              ...
                              if (typeof dateStr === 'number' || !dateStr.includes('-')) {
                                ...
                                dn = d.toLocaleDateString('en-US', { weekday: 'short' });
                                dayNum = String(day);
                                ...
                                const [y, m, d] = dateStr.split('-');
                                const dateObj = new Date(Number(y), Number(m) - 1, Number(d));
                                dn = dateObj.toLocaleDateString('en-US', { weekday: 'short', month: 'short' });
                                dayNum = String(Number(d));
                                fullDateStr = dateStr;
                              }
                              
                              const obsOnDay = observations.filter(o =>
                                o.positionId === selectedPositionId && o.date === fullDateStr
                              ).length;
                              
                              return (
                                <button key={dateStr}
                                  onClick={() => setNewObservation(prev => ({ ...prev, date: fullDateStr }))}
                                  ...
                                  {dn} {dayNum}
                                  {obsOnDay > 0 && <span className="ml-1 opacity-70">({obsOnDay})</span>}
                                </button>
                              );
                            })}
                          </div>
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {lockedTimes.map((t, i) => {
                              const obsAtTime = observations.filter(o =>
                                o.positionId === selectedPositionId &&
                                o.date === newObservation.date &&
                                o.time === t
                              ).length;
                              return (
                                <button key={i}
                                  onClick={() => setNewObservation(prev => ({ ...prev, time: t }))}
                      ...
                      <p className="text-xs text-indigo-500 mt-2">Click a date or time to pre-fill the form below. Green = already has observations.</p>
                    </div>
                  ) : !paramsSaved ? (
                    <div className="mb-6 p-4 rounded-lg border border-amber-200 bg-amber-50">
                      <p className="text-sm text-amber-800">
                        ...
                        <button onClick={() => setActiveTab('parameters')} className="underline font-semibold">Study Parameters</button>
                    ...
                    <div className="mb-6 p-4 rounded-lg border border-amber-200 bg-amber-50">
                      <p className="text-sm text-amber-800 mb-3">
                        ...
                        Study parameters are saved, but the observation schedule was not locked.
                        Click below to generate and lock the schedule so you can record observations.
                      </p>
                      <button
                        onClick={saveAndLockParameters}
                        ...
                        style={{ backgroundColor: '#322b80' }}>
                        {saveStatus === 'saving' ? 'Saving…' : 'Lock Schedule Now'}
                      </button>
                    </div>
                  )}
                  {/* Position selector pills */}
                  <div className="flex flex-wrap gap-2 mb-5">
                    {positions.filter(p => p.dbId).map(pos => (
                      <button key={pos.id}
                        onClick={() => setSelectedPositionId(pos.id)}
                        ...
                        <span className="ml-1.5 opacity-70">
                          ({observations.filter(o => o.positionId === pos.id).length})
                        </span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid md:grid-cols-6 gap-3 mb-6 p-5 bg-gray-50 rounded-lg">
                    ...
                    <div className="md:col-span-2 px-4 py-3 border border-indigo-200 bg-indigo-50 rounded-lg text-sm font-medium text-indigo-800 flex items-center gap-2">
                      <Users size={14} />
                      {selectedPosition ? selectedPosition.name : <span className="text-gray-400">Select a position above</span>}
                    </div>

                    {/* Date — dropdown of locked study dates */}
                    <select
                      value={newObservation.date}
                      onChange={e => setNewObservation(p => ({ ...p, date: e.target.value, time: '' }))}
                    ...
                    <select
                      ...
                      onChange={e => setNewObservation(p => ({ ...p, time: e.target.value }))}
                      disabled={!selectedPosition || !paramsSaved || lockedTimes.length === 0}
                      className="px-4 py-3 border border-gray-300 rounded-lg bg-white disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed">
                      <option value="">— Select time —</option>
                      {lockedTimes.map((t, i) => {
                        const alreadyRecorded = observations.some(o =>
                          o.positionId === selectedPositionId &&
                          o.date === newObservation.date &&
                          o.time === t
                        );
                    ...
                    {/* Busy / Not Busy */}
                    <select value={newObservation.isBusy.toString()}
                      onChange={e => setNewObservation(p => ({ ...p, isBusy: e.target.value === 'true' }))}
                    ...
                    {/* Performance rating */}
                    <input type="number" placeholder="Performance Rating %" min={0} max={200}
                      value={newObservation.performanceRating}
                      onChange={e => setNewObservation(p => ({ ...p, performanceRating: Number(e.target.value) }))}
                      className="px-4 py-3 border border-gray-300 rounded-lg" />

                    {(() => {
                      const exists = observations.some(o =>
                        o.positionId === selectedPositionId &&
                        o.date === newObservation.date &&
                        o.time === newObservation.time
                      );
                      return (
                        <button
                          ...
                          disabled={!selectedPosition || !newObservation.date || !newObservation.time}
                          ...
                          style={{ backgroundColor: exists ? '#16a34a' : '#322b80' }}>
                    ...
                    {error && <div className="md:col-span-6 text-red-500 text-sm">{error}</div>}
                  </div>
                      ...
                      <tbody>
                        {(() => {
                          const filtered = selectedPositionId
                            ? observations.filter(o => o.positionId === selectedPositionId)
                            : observations;
                          if (filtered.length === 0) return (
                            <tr><td colSpan={6} className="p-8 text-center text-gray-400">
                              No observations recorded for {selectedPosition?.name ?? 'this position'} yet.
                            </td></tr>
                          );
                          return filtered.map(obs => {
                            const pos = positions.find(p => p.id === obs.positionId);
                            return (
                              <tr key={obs.id} className="border-b border-gray-100 hover:bg-gray-50">
                                ...
                                <td className="p-3 font-mono">{obs.time}</td>
                                <td className="p-3">
                                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${obs.isBusy ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-700'}`}>
                                ...
                                <td className="p-3">
                                  {!obs.dbId
                                    ...
                                    : <button onClick={() => removeObservation(obs.id)}
                    ...
                    <div className="text-sm text-gray-500">
                      {(() => {
                        ...
                        return posObs.length > 0 ? (
                          <span className="flex items-center gap-1.5 text-green-600">
                            <CheckCircle2 size={14} /> {posObs.length} observation{posObs.length !== 1 ? 's' : ''} recorded{selectedPosition ? ` for ${selectedPosition.name}` : ''}
                          </span>
                        ) : (
                          <span className="flex items-center gap-1.5 text-amber-600">
                            <AlertCircle size={14} /> No observations recorded yet
                          </span>
                        );
                      })()}
                    </div>
          ...
          {activeTab === 'analysis' && (
            <div>
              <div className="mb-6">
                ...
                {selectedPosition && (
                  <p className="text-sm text-gray-500 mt-1">
                    Row for <span className="font-semibold text-indigo-700">{selectedPosition.name}</span> is highlighted.
                  </p>
                )}
              </div>
              ...
              {analysisResults.length === 0 ? (
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid md:grid-cols-5 gap-4">
                    {[
                      { label: 'Total Positions', val: positions.length, color: '#322b80' },
                      { label: 'Total Observations', val: observations.length, color: '#16a34a' },
                      ...
                      { label: 'Avg Performance',
                        ...
                        color: '#7c3aed' },
                      { label: 'TAM (Σ ESMᵢ)', val: Math.round(TAM).toLocaleString(), color: '#d97706' },
                    ].map(({ label, val, color }) => (
                      <div key={label} className="p-5 rounded-lg text-white" style={{ backgroundColor: color }}>
                    ...
                    <table className="w-full border-collapse text-sm">
                      ...
                      <tbody>
                        {analysisResults.map(r => (
                          <tr key={r.positionId}
                            onClick={() => setSelectedPositionId(r.positionId)}
                            className={`border-b border-gray-100 cursor-pointer transition-colors ${
                              r.positionId === selectedPositionId
                                ? 'bg-indigo-50 font-medium'
                                : 'hover:bg-gray-50'
                            }`}>
                            <td className="p-3 font-medium">{r.positionName}</td>
                            <td className="p-3 text-right">{r.totalObservations}</td>
                            <td className="p-3 text-right">{r.busyCount}</td>
                            <td className="p-3 text-right">
                              <div className="flex items-center justify-end gap-2">
                                <div className="w-16 bg-gray-200 rounded-full h-1.5">
                                  <div className="h-1.5 rounded-full" style={{ width: `${Math.min(r.utilizationFactor, 100)}%`, backgroundColor: '#322b80' }} />
                                </div>
                                <span className="font-medium">{r.utilizationFactor.toFixed(1)}%</span>
                              </div>
                            </td>
                            <td className="p-3 text-right">{r.avgPerformanceRating.toFixed(1)}%</td>
                            <td className="p-3 text-right">{r.performanceAllowance}%</td>
                            <td className="p-3 text-right">{r.estimatedAnnualManHours.toFixed(1)}</td>
                            <td className="p-3 text-right">{r.estimatedBasicManHours.toFixed(1)}</td>
                            <td className="p-3 text-right font-bold" style={{ color: '#322b80' }}>{r.estimatedStandardManHours.toFixed(1)}</td>
                          </tr>
                        ))}
                        <tr className="border-t-2 font-bold" style={{ backgroundColor: '#f5f3ff' }}>
                          <td className="p-3" colSpan={8} style={{ color: '#322b80' }}>TAM = Σ ESMᵢ (eq 6.15)</td>
                          <td className="p-3 text-right text-lg" style={{ color: '#322b80' }}>{TAM.toFixed(1)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="p-4 rounded-lg border bg-gray-50 text-xs text-gray-600 space-y-1">
                    <p className="font-semibold text-sm mb-2" style={{ color: '#322b80' }}>Formulas (Charles-Owaba)</p>
                    <p><code>Uᵢ</code> = Busy / Total  <span className="text-gray-400">(eq 6.11)</span></p>
                    <p><code>EAMᵢ</code> = Uᵢ × Available Annual Hours  <span className="text-gray-400">(eq 6.12)</span></p>
                  ...
                  <div className="flex justify-end">
                    <button onClick={() => {
                      const blob = new Blob([JSON.stringify({ studyDbId, positions, observations, studyParameters, analysisResults, TAM, exportDate: new Date().toISOString() }, null, 2)], { type: 'application/json' });
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6 animate-in fade-in zoom-in duration-200">
              <div className="flex items-center gap-3 text-red-600 mb-4">
                ...
                <h3 className="text-lg font-bold">Delete Study #{deleteConfirmId}?</h3>
              </div>
              <p className="text-gray-600 mb-6 text-sm">
                This action is irreversible and will permanently remove this study, along with all associated positions and observations.
              </p>
