    ...
    <div className="p-8 w-full max-w-6xl mx-auto">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <h1 className="text-2xl font-bold text-strong">Appraisal History</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-line p-12 text-center">
          ...
          <p className="text-muted">Run the Non-Academic Appraisal model and save results to see them here.</p>
        </div>
      ) : (
          ...
          {history.map((run) => (
            <div key={run.id} className="bg-white rounded-xl shadow-sm border border-line overflow-hidden">
              ...
              <div className="p-6">
                ...
                <h4 className="text-sm font-semibold text-body mb-3 uppercase tracking-wider">Metrics Breakdown</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  <div className="bg-canvas rounded-lg p-3 border border-line text-center">
                    <p className="text-xs text-muted mb-1">Output Quantity</p>
                    <p className="text-lg font-bold text-strong">{Number(run.output).toFixed(1)}</p>
                  </div>
                  <div className="bg-canvas rounded-lg p-3 border border-line text-center">
                    <p className="text-xs text-muted mb-1">Quality of Work</p>
                    <p className="text-lg font-bold text-strong">{Number(run.quality).toFixed(1)}</p>
                  </div>
                  <div className="bg-canvas rounded-lg p-3 border border-line text-center">
                    <p className="text-xs text-muted mb-1">Efficiency</p>
                    <p className="text-lg font-bold text-strong">{Number(run.efficiency).toFixed(1)}</p>
                  </div>
                  <div className="bg-canvas rounded-lg p-3 border border-line text-center">
                    <p className="text-xs text-muted mb-1">Attendance</p>
                    <p className="text-lg font-bold text-strong">{Number(run.attendance).toFixed(1)}</p>
                  </div>
                  <div className="bg-canvas rounded-lg p-3 border border-line text-center">
                    <p className="text-xs text-muted mb-1">Teamwork</p>
                    <p className="text-lg font-bold text-strong">{Number(run.teamwork).toFixed(1)}</p>
                  </div>
                </div>
              </div>
            </div>
