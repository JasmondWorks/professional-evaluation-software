    ...
    <div className="p-8 w-full max-w-7xl mx-auto">
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          >
            <ArrowLeft2 size="16" className="mr-1" /> Back to Stress Evaluation Model
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">Stress Evaluation History</h1>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500">Run the Stress Evaluation model and save results to see them here.</p>
        </div>
      ) : (
        <>
          <HistoryChart 
          ...
          {history.map((run) => {
            const parsedAnova = run.anova_result ? JSON.parse(run.anova_result) : null;
                ...
                <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  {parsedAnova && (
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 h-full flex flex-col">
                      <h4 className="text-sm font-bold text-gray-900 mb-3 border-b border-gray-200 pb-2">ANOVA Test Results</h4>
                      <div className="space-y-2 flex-1">
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">F-Statistic:</span>
                          <span className="text-sm font-semibold">{parsedAnova.fStatistic?.toFixed(3)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-sm text-gray-600">Critical Value:</span>
                          <span className="text-sm font-semibold">{parsedAnova.criticalValue}</span>
                        </div>
                      </div>
                      <div className={`mt-3 p-2 rounded-md text-xs font-bold text-center border ${
