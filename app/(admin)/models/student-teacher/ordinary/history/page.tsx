...
export default function OrdinaryHistoryPage() {
  ...
  useEffect(() => {
    async function fetchHistory() {
      try {
        ...
        const res = await apiFetch("/api/results?mode=ordinary", {
          method: "GET",
          headers: {
  ...
  return (
    <div className="p-8 w-full mx-auto">
      <div className="mb-8">
        <BackLink href="/models/student-teacher/ordinary" className="mb-4">Back to Ordinary Model</BackLink>
        <h1 className="text-2xl font-bold mb-2">Ordinary Optimization History</h1>
        <p className="text-body">
          Historical records of your ordinary student-teacher ratio calculations.
        </p>
      </div>
      ...
      {error ? (
        ...
        <div className="bg-white border rounded-xl p-12 text-center shadow-sm">
          ...
          <p className="text-muted">You haven't saved any Ordinary Optimization results yet.</p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-line">
                {runs.map((run) => (
                  <tr key={run.id} className="hover:bg-canvas transition-colors">
                    ...
                    <td className="px-6 py-4 font-semibold text-strong text-center">
                      {run.totalStaffNeeded}
                    </td>
                    <td className="px-6 py-4 text-body text-center">
