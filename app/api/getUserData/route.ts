...
async function getUserData(user: string | null, view?: string) {
  if (view === "team") {
    const [goodPerformance, badPerformance] = await Promise.all([
      prisma.performance.findMany({ where: { type: "good" } }),
      prisma.performance.findMany({ where: { type: "bad" } }),
    ]);
...
export async function POST(request: NextRequest) {
  const { token, view } = await request.json();
  ...
  if (token) {
    try {
      let { goodPerformance, badPerformance } = await getUserData(
