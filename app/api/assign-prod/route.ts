...
export async function POST(req: NextRequest) {
  try {
    ...
    const mailOptions = {
      ...
      to: user.email,
      subject: "You have been assigned as Production Engineer",
      text: `Dear ${user.name},\n\nYou have been officially assigned as Production Engineer at ${org}.\n\nCongratulations!\n\nBest,\nAdmin`,
    };

    await transporter.sendMail(mailOptions);

    return NextResponse.json({
      message: `Successfully assigned ${user.name} as Production Engineer and sent email.`,
      user,
    });
  } catch (error: any) {
