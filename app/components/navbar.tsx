            ...
            {recent.length === 0 ? (
              ...
              <ul className="max-h-96 overflow-y-auto divide-y divide-line">
                {recent.map((n) => (
                  <li
                    key={n.id}
                    className={`px-4 py-3 flex gap-3 ${n.is_read ? "" : "bg-pes-50/50"}`}
                  >
                    <span
                      className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${n.is_read ? "bg-transparent" : "bg-pes"}`}
                    />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="flex flex-col gap-0.5 py-2">
              <span className="text-sm font-semibold text-strong normal-case truncate">
                {name || "Account"}
              </span>
              {role && (
                <span className="text-xs text-muted capitalize font-normal">
                  {role.replace(/-/g, " ")}
                </span>
              )}
            </DropdownMenuLabel>
