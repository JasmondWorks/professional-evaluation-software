...
function Item({ label, level }: { label: string; level: Level }) {
  return (
    <div className="rounded-lg border border-line bg-canvas p-5 text-center">
      <p className="text-xs font-semibold text-muted uppercase tracking-wider mb-1">{label}</p>
      {level?.name && <p className="text-[11px] text-muted mb-1 truncate">{level.name}</p>}
      {level && level.stress !== null ? (
        <p className="text-3xl font-bold text-rose-600">{level.stress.toFixed(1)}%</p>
      ) : (
        <p className="text-xs text-muted mt-2">No result for the current cycle yet.</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="mx-6 mb-6 bg-white rounded-lg shadow-md shadow-gray-100 p-6">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-xl font-bold text-strong">My Stress Results</h2>
        <Link href="/data-entry/stress/my-results" className="text-sm text-pes hover:underline">
          View details
        </Link>
      </div>
      <p className="text-sm text-muted mb-4">Shared with you by your organization — group-level results only.</p>
